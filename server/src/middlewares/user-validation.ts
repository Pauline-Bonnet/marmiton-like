...
export const userCreationValidationRules = [
    body('firstname')
    .isString()
    .isLength({min:1, max: 50}).withMessage('Prénom max 50 caractères')
    .notEmpty().withMessage('Champ prénom requis'),
    
    body('lastname')
    .isString()
    .isLength({min:1, max: 100}).withMessage('Prénom au max 100 caractères')
    .notEmpty().withMessage('Champ nom requis'),

    body('pseudo')
    .isString()
    .isLength({min:1, max: 50}).withMessage('Pseudo max 50 caractères')
    .notEmpty().withMessage('Champ pseudo requis'),
